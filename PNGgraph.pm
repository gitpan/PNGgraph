...
$PNGgraph::VERSION = '1.11';

# Some tools and utils
use PNGgraph::colour qw(:colours);
...

{
    ...
    sub put_logo($) # GD::Image
	{
        ...
        LOGO: for ($self->{logo_position}) {
            ...
            /UR/i && do {
                $x = $self->{pngx} - $r_margin - $w * $r;
                $y = $self->{t_margin};
                last LOGO;
            };
            /LL/i && do {
                $x = $self->{l_margin};
                $y = $self->{pngy} - $b_margin - $h * $r;
                last LOGO;
            };
            # default "LR"
            $x = $self->{pngx} - $r_margin - $r * $w;
            $y = $self->{pngy} - $b_margin - $r * $h;
            last LOGO;
        }
...
=item C<PNGgraph::lines>

Create a line chart.

=item C<PNGgraph::bars>

Create a bar chart.

=item C<PNGgraph::points>

Create an chart, displaying the data as points.

=item C<PNGgraph::linespoints>

Combination of lines and points.

=item C<PNGgraph::area>

Create a graph, representing the data as areas under a line.

=item C<PNGgraph::mixed>
...
use B<undef>, and I<PNGgraph> will skip that point.

Create a new I<PNGgraph> object by calling the I<new> operator on the
...
L<PNGgraph::colour> (C<S<perldoc PNGgraph::colour>> for the names available).
...
the data points, or I<PNGgraph> will die with a message.
...
First of all: PNGgraph does B<not> support numerical x axis the way it
...
C<x_tick_number> is set to a defined value, PNGgraph will attempt to
...
If set to I<'auto'>, PNGgraph will attempt to format the X axis in a
